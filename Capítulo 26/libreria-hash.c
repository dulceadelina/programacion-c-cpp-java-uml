...
void creacion(void);
void compra(void);      /** operación de dar de ALTA */
void venta(void);       /** operación de dar de BAJA */
void consulta(void);
...
/** Alta de un registro: pide al usuario los campos código, título y autor. Llama a
    ...
    colisiones la primera posición libre que será donde escribe el regsitro. */
void compra(void)
{
    Libro lib, libar;
    long posicion;
    ...
    {
        puts("código repetido, rebisar los datos.");
        return;
    }
    else
        colisiones(lib);
...
/** Baja de un registro: pide el código del registro. Se lee el registro cuya
    ...
    se da de baja escribiendo '*' en el campo código. En caso contrario se busca
    en el área de colisiones y se procede igual */
void venta(void)
{
    Libro libar;
    char codigo[7], r;
...
/** Aritmética modular: transforma cadena a un entero en el rango [0, principal).
    ...
    a a la función que convierte la cadena a entero largo. Por último, aplica el módulo
...
long transformaClave(const char clave[])
{
    ...
    for(j = 0; j < strlen(clave); j++)
        d = d *27 + clave[j];
    /** Si d supera el máximo entero largo, genera un número negativo */
    if(d < 0) d = -d;
    return d;
}
void escribir(Libro lib)
